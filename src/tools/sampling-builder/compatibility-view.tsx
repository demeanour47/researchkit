import { Tag } from "@/ui";
import { CHECK_STATUS_LABELS, checkSamplingCompatibility, getTechnique, samplingSupport, type CheckStatus, type ResearchProjectDraft, type SamplingTechniqueId } from "@/knowledge/research";
import { steps } from "./copy";

export const tones: Record<CheckStatus, "info" | "neutral" | "caution"> = { aligned: "info", review: "neutral", "worth-checking": "caution", missing: "caution", clarify: "caution" };

/** One technique's fit with the project: why it could support it, every check, and its assumptions. */
export function CompatibilityView({ id, project }: { id: SamplingTechniqueId; project: ResearchProjectDraft }) {
  const technique = getTechnique(id);
  const support = samplingSupport(id, project);
  ...
  return (
    <section aria-labelledby={headingId} className="grid gap-4 rounded-panel border border-border bg-surface p-4 sm:p-6">
      ...
      <ul className="grid gap-3">
        {checkSamplingCompatibility(id, project).map((check) => (
      ...
      <div className="grid gap-2">
        ...
        <ul className="grid list-disc gap-1 ps-6">
          {technique.assumptions.map((assumption) => (
